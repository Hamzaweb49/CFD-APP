...
class GadgetFormScreen extends StatefulWidget {
  const GadgetFormScreen({super.key});

  @override
  State<GadgetFormScreen> createState() => _GadgetFormScreenState();
}

class _GadgetFormScreenState extends State<GadgetFormScreen> {
  GadgetFormScreenController gadgetFormScreenController =
      Get.put(GadgetFormScreenController());
  ...
  Future getData() async {
    if (isFrom == 'EditScreen') {
      gadgetFormScreenController.getAllGadgetData = Get.arguments['ModelData'];
    }
    if (selectedData.subCategoryName == 'Mobiles') {
      await gadgetFormScreenController.getMobileData();
    } else if (selectedData.subCategoryName == 'Tablets') {
      await gadgetFormScreenController.getTabletData();
    }
    if (preferences
      ...
      gadgetFormScreenController.nameController.text =
          userLoginOtpResponseModel?.firstName ?? '';
      gadgetFormScreenController.phoneController.text =
  ...
  void initState() {
    ...
    getData();
    gadgetFormScreenController.clearData();
    if (isFrom == 'EditScreen') {
      gadgetFormScreenController.assignData();
    }
    gadgetFormScreenController.getStateData();
    brandselect = gadgetFormScreenController.brandSelect;
    super.initState();
  }

  @override
  void dispose() {
    gadgetFormScreenController.getAllNearByResponseModel = [];
    gadgetFormScreenController.getAllCityResponseModel = [];
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppContainer(
      ...
      child: Scaffold(
        ...
        body: GetBuilder<GadgetFormScreenController>(
          builder: (controller) {
            return
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    (h * 0.02).addHSpace(),
                    selectedData.subCategoryName == "Tablets" ||
                            selectedData.subCategoryName == "Mobiles"
                        ? AutoCorrectField(
                            ...
                            optionList:
                                selectedData.subCategoryName.toString() ==
                                    ...
                                    : controller.tabletList,
                            label: 'Brand',
                            onSelected: (selection) async {
                              debugPrint('You just selected$selection');
                              controller.brandSelect = selection;

                              await controller.getBrandId(
                                  category: selectedData.subCategoryName,
                              ...
                              onPressed: () async {
                                ...
                                if (controller.confirmLocation == 0) {
                                  ...
                                  if (controller.stateSelect.isEmpty) {
                                    ...
                                    controller.isValidate = true;
                                  } else if (controller.nearBySelect.isEmpty) {
                                    controller.isValidate = true;
                                  }
                                }
                                ...
                                if (controller.titleController.text.isEmpty) {
                                  ...
                                  showErrorSnackBar(AppString.enterpriceText);
                                } else if (controller.addDataList.isEmpty) {
                                  showErrorSnackBar(AppString.selectImageText);
                                  ...
                                  List<Map<String, dynamic>> gadgetImageList =
                                      [];
                                  await controller.uploadImages(
                                      requestData: controller.addDataList);

                                  if (controller.imageUrl.isNotEmpty) {
                                    for (var element in controller.imageUrl) {
                                      gadgetImageList.addAll({
                                        {
                                          "id": 0,
                                          "imageId": "",
                                          "imageURL": element,
                                  ...
                                  Map<String, dynamic> gadgetData = {
                                    ...
                                    "isPremium": isFrom == 'EditScreen'
                                        ? controller
                                            .getAllGadgetData[0].isPremium
                                        : false,
                                    "isActive": isFrom == 'EditScreen'
                                        ? controller
                                            .getAllGadgetData[0].isActive
                                        : false,
                                    "isVerified": isFrom == 'EditScreen'
                                        ? controller
                                            .getAllGadgetData[0].isVerified
                                        : false,
                                    ...
                                    "tableRefGuid": isFrom == 'EditScreen'
                                        ? controller
                                            .getAllGadgetData[0].tableRefGuid
                                        : "",
                                    "id": isFrom == 'EditScreen'
                                        ? controller.getAllGadgetData[0].id
                                        : 0,
                                    "gadgetImageList": gadgetImageList,
                                    "subCategoryId": selectedData.id,
                                    "mobileBrandId":
                                        controller.selectedBrandID ?? 0,
                                  };
                                  if (isFrom == 'FormScreen') {
                                    await controller.addGadgetData(
                                        gadgetData: gadgetData);
                                    // log('ADD DATA------>>>${await controller.addGadgetData(gadgetData: gadgetData)}');
                                  } else {
                                    controller.updateGadgetData(
                                        gadgetData: gadgetData,
                                        id: controller.getAllGadgetData[0].id
                                            .toString());
                                    log('UPDATE DATA');
                                  }
                                }
