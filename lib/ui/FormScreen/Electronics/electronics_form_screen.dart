...
import 'package:claxified_app/ui/FormScreen/Electronics/electronics_form_controller.dart';
...
class ElectronicsFormScreen extends StatefulWidget {
  const ElectronicsFormScreen({super.key});

  @override
  State<ElectronicsFormScreen> createState() => _ElectronicsFormScreenState();
}

class _ElectronicsFormScreenState extends State<ElectronicsFormScreen> {
  ElectronicsFormScreenController electronicsFormScreenController =
      Get.put(ElectronicsFormScreenController());
  ...
  Future getData() async {
    if (Get.arguments['isFrom'] == 'EditScreen') {
      electronicsFormScreenController.getAllElectronicsData =
          Get.arguments['ModelData'];
    }

    await electronicsFormScreenController.getStateData();
    if (preferences
      ...
      electronicsFormScreenController.nameController.text =
          userLoginOtpResponseModel?.firstName ?? '';
      electronicsFormScreenController.phoneController.text =
  ...
  void initState() {
    getData();
    electronicsFormScreenController.clearData();
    if (Get.arguments['isFrom'] == 'EditScreen') {
      electronicsFormScreenController.assignData();
    }
    super.initState();
  }
  ...
  @override
  void dispose() {
    electronicsFormScreenController.getAllNearByResponseModel = [];
    electronicsFormScreenController.getAllCityResponseModel = [];
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppContainer(
      child: GetBuilder<ElectronicsFormScreenController>(
        builder: (controller) {
          return SafeArea(
            child: Scaffold(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ReorderableGridView.builder(
                        ...
                        itemCount:
                            controller.addElectronicsImageList.length + 1,
                        itemBuilder: (BuildContext context, int index) {
                          if (index ==
                              controller.addElectronicsImageList.length) {
                            return GestureDetector(
                              key: const ValueKey('add_button'),
                              onTap: () {
                                if (controller.addElectronicsImageList.length <=
                                    9) {
                              ...
                              child: controller
                                          .addElectronicsImageList.length ==
                                      10
                                  ? const SizedBox()
                                  : Container(
                                      height: h * 0.1,
                            ...
                            return GridTile(
                              key: ValueKey(
                                  controller.addElectronicsImageList[index]),
                              child: Container(
                                height: h * 0.14,
                                width: h * 0.14,
                                decoration: BoxDecoration(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (controller
                                      ...
                                      showErrorSnackBar(
                                        ...
                                        .addElectronicsImageList.isEmpty) {
                                      showErrorSnackBar(
                                      ...
                                      await controller.electronicsUploadImages(
                                          requestData: controller
                                              .addElectronicsImageList);

                                      if (controller
                                          .imageElectronicsUrl.isNotEmpty) {
                                        for (var element
                                            in controller.imageElectronicsUrl) {
                                          electronicApplianceImageList.addAll({
                                            {
                                              "id": 0,
                                              "imageId": "",
                                              "imageURL": element,
                                              "electronicAppliancesId": 0
                                            }
                                          });
                                        }
                                      }
                                      ...
                                      if (Get.arguments['isFrom'] ==
                                          'FormScreen') {
                                        await controller.addElectronicsData(
                                            electronicsData: electronicsData);
                                      } else {
                                        controller.updateElectronicsData(
                                            id: controller
                                                .getAllElectronicsData[0].id
                                                .toString(),
