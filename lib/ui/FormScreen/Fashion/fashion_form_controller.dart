...
class FashionFormController extends GetxController {
  ...
  /// ADD NEW FASHION DATA API
  addFashionData({required Map<String, dynamic> fashion}) async {
    isLoading.value = true;
    ResponseItem result;
    result = await FashionRepo.addFashionRepo(fashion: fashion);
    try {
  ...
  uploadImages({required List<File> requestData}) async {
    isLoading.value = true;
    ResponseItem result;
    result = await FashionRepo.uploadImagesRepo(requestData: requestData);
    try {
      if (result.status) {
        if (result.data != null) {
          isLoading.value = false;
  ...
  getPinCodeData({required String pinCode}) async {
    ResponseItem result;
    result = await FashionRepo.pincodeRepo(pincode: pinCode);
    try {
      if (result.status) {
        if (result.data != null) {
  ...
  /// ASSIGN DATA
  List<GetAllFashionDataResponseModel> getAllFashionData = [];
  assignData() {
    if (getAllFashionData.isNotEmpty) {
      saveImage(fashionImageList: getAllFashionData[0].fashionImageList);

      /// Pincode Api
      getPinCodeData(pinCode: getAllFashionData[0].pincode ?? '');
      titleController.text = getAllFashionData[0].title ?? '';
      stateController.text = getAllFashionData[0].state ?? '';
      stateSelect = getAllFashionData[0].state ?? '';
      cityController.text = getAllFashionData[0].city ?? '';
      citySelect = getAllFashionData[0].city ?? '';
      nearByController.text = getAllFashionData[0].nearBy ?? '';
      nearBySelect = getAllFashionData[0].nearBy ?? '';
      descriptionController.text = getAllFashionData[0].discription ?? '';
      priceController.text = getAllFashionData[0].price.toString() ?? '';
      pinCodeController.text = getAllFashionData[0].pincode ?? '';
      nameController.text = getAllFashionData[0].name ?? '';
      phoneController.text = getAllFashionData[0].mobile ?? '';
    }
    update();
  }

  /// SAVE ASSIGN IMAGES
  saveImage({required List<FashionImageList>? fashionImageList}) async {
    fashionImageList?.forEach((element) async {
  ...
  /// put Fashion
  updateFashionAllData(
      {required Map<String, dynamic> updateFashionData,
      required String id}) async {
    isLoading.value = true;
    ResponseItem result;
    result = await UpdateFashionData.updateFashionData(
        updateData: updateFashionData, id: id);
    try {
      if (result.status == true) {
