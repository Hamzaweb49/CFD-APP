...
class SportsFormController extends GetxController {
  ...
  /// ADD NEW SPORTS DATA API
  addSportsData({required Map<String, dynamic> sports}) async {
    isLoading.value = true;
    ResponseItem result;
    result = await SportsRepo.addSportsRepo(sports: sports);
    try {
  ...
  uploadImages({required List<File> requestData}) async {
    isLoading.value = true;
    ResponseItem result;
    result = await SportsRepo.uploadImagesRepo(requestData: requestData);
    try {
      if (result.status) {
        if (result.data != null) {
          isLoading.value = false;
  ...
  getPinCodeData({required String pinCode}) async {
    ResponseItem result;
    result = await SportsRepo.pincodeRepo(pincode: pinCode);
    try {
      if (result.status) {
        if (result.data != null) {
  ...
  /// ASSIGN DATA
  List<GetAllSportsDataResponseModel> getAllSportsData = [];
  assignData() {
    if (getAllSportsData.isNotEmpty) {
      saveImage(sportImageList: getAllSportsData[0].sportImageList);

      /// Pincode Api
      getPinCodeData(pinCode: getAllSportsData[0].pincode ?? '');
      titleController.text = getAllSportsData[0].title ?? '';
      stateController.text = getAllSportsData[0].state ?? '';
      stateSelect = getAllSportsData[0].state ?? '';
      cityController.text = getAllSportsData[0].city ?? '';
      citySelect = getAllSportsData[0].city ?? '';
      nearByController.text = getAllSportsData[0].nearBy ?? '';
      nearBySelect = getAllSportsData[0].nearBy ?? '';
      descriptionController.text = getAllSportsData[0].discription ?? '';
      priceController.text = getAllSportsData[0].price.toString() ?? '';
      pinCodeController.text = getAllSportsData[0].pincode ?? '';
      nameController.text = getAllSportsData[0].name ?? '';
      phoneController.text = getAllSportsData[0].mobile ?? '';
    }
    update();
  }

  /// SAVE ASSIGN IMAGES
  saveImage({required List<SportImageList>? sportImageList}) async {
    sportImageList?.forEach((element) async {
  ...
  /// put Sports
  updateSportsAllData(
      {required Map<String, dynamic> sportsData, required String id}) async {
    isLoading.value = true;
    ResponseItem result;
    result =
        await UpdateSportsData.updateSportsData(updateData: sportsData, id: id);
    try {
      if (result.status == true) {
